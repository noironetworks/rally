...
class EastWest(neutron_utils.NeutronScenario, gbputils.GBPScenario, osutils.OSScenario, nova_utils.NovaScenario,
    ...
    def run(self, controller_ip, image, flavor):
        ...
        ptg1 = self.create_gbp_policy_target_group(gbp, "demo_diff_ptg_l2p_same_l3p_ptgs[0]", **{"l2_policy_id": l2p1})
        ptg2 = self.create_gbp_policy_target_group(gbp, "demo_diff_ptg_l2p_same_l3p_ptgs[1]", **{"l2_policy_id": l2p2})

        pt1, port1 = self.create_gbp_policy_target(gbp, "vm7_data_pt", "demo_diff_ptg_l2p_same_l3p_ptgs[0]", 1)
        pt2, port2 = self.create_gbp_policy_target(gbp, "vm8_data_pt", "demo_diff_ptg_l2p_same_l3p_ptgs[1]", 1)
        pt3, port3 = self.create_gbp_policy_target(gbp, "vm9_data_pt", "demo_diff_ptg_l2p_same_l3p_ptgs[1]", 1)

        vm7 = self.boot_server(port1, key_name, image, flavor)
        vm8 = self.boot_server(port2, key_name, image, flavor)
        vm9 = self.boot_server(port3, key_name, image, flavor)
        ...
        self.traffic_with_all_rules(gbp, policy_rule_set)
