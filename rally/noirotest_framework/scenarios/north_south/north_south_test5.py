...
class NorthSouth(create_resources.CreateResources, osutils.OSScenario, gbputils.GBPScenario, neutron_utils.NeutronScenario,
    ...
    def run(self, controller_ip, image, flavor, L3OUT1, L3OUT1_NET, L3OUT1_VRF, L3OUT2,
        ...
        print "Execution of Testcase TEST_NAT_FUNC_5 starts\n"
        ...
        print "Create Policy Target group with Default L3\n"
        ...
        print "Create Policy Target group with Created L3\n"
        ...
        print "Create Another External Segment "+L3OUT2+"\n"
        ext_seg2 = self.create_gbp_external_segment(gbp, L3OUT2,
                                                   **{"subnet_id": ext_sub_list[2].get("subnet")["id"],
                                                      "external_routes": [{"destination": "0.0.0.0/0", "nexthop": None}],
                                                      "shared": True})
        ...
        print "Associate NAT pool with new External Segment\n"
        self.update_gbp_nat_pool(gbp, nat_pool, **{"external_segment_id": ext_seg2})

        print "Create ExtPolicy for new ExtSegment and Apply PolicyRuleSets\n"
        ext_pol2 = self.create_gbp_external_policy(gbp, L3OUT2_NET, **{"external_segments": [ext_seg2]})
        self.update_gbp_external_policy(gbp, ext_pol2, "uuid", [policy_rule_set["icmp_tcp_id"]])

        print "Associate FIPs to VMs\n"
        fip3 = self.admin_attach_floating_ip(vm1, ext_net_list[1].get("network"))
        fip4 = self.admin_attach_floating_ip(vm2, ext_net_list[1].get("network"))
        self.sleep_between(10, 15)

        self.add_route_in_extrtr(ext_rtr, "50.50.50.0/24", gwip2_extrtr, "update")
