...
class NorthSouth(create_resources.CreateResources, osutils.OSScenario, gbputils.GBPScenario, neutron_utils.NeutronScenario,
    ...
    def run(self, controller_ip, image, flavor, L3OUT1, L3OUT1_NET, L3OUT1_VRF, L3OUT2,
        ...
        print "Execution of Testcase TEST_NAT_FUNC_1 starts\n"
        ...
        self.admin_dissociate_floating_ip(vm1, fip1)
        self.admin_dissociate_floating_ip(vm2, fip2)
        self.sleep_between(10, 15)
        print "Statically Inter-Changing Association of FIPs to VMs\n"
        self.admin_associate_floating_ip(vm1, fip2)
        self.admin_associate_floating_ip(vm2, fip1)
        self.sleep_between(10, 15)

        print "DNATed Traffic from ExtRTR to VMs after Inter-Change of FIPs\n ICMP and TCP test from external router"
        self._remote_command_wo_server("noiro", "noir0123", ext_rtr, command1)
        self._remote_command_wo_server("noiro", "noir0123", ext_rtr, command2)

        self.cleanup_ns(gbp, [vm1, vm2], [fip2, fip1], ext_net_list)
