...
class NorthSouth(neutron_utils.NeutronScenario, gbputils.GBPScenario, osutils.OSScenario, nova_utils.NovaScenario,
    ...
    def run(self, controller_ip, image, flavor):
        ...
        ext_net_list, ext_sub_list, vm_list, ext_seg, nat_pool1 = self.create_resources_for_north_south_tests(gbp_ad, gbp, key_name,
                                                                                          image, flavor, nat_pool2=True)
        ...
        self.delete_gbp_nat_pool(gbp, nat_pool1)

        nat_pool2 = self.create_gbp_nat_pool(gbp, "NatPoolTest2",
                                             **{"ip_pool": "60.60.60.0/24", "external_segment_id": ext_seg})
        fip3 = self._attach_floating_ip(vm_list[0], ext_net_list[0])
        fip4 = self._attach_floating_ip(vm_list[1], ext_net_list[0])
        self.sleep_between(10, 15)
