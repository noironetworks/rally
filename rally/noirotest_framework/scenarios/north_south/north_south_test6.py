...
@scenario.configure(name="ScenarioPlugin.north_south_test6", context={"cleanup@openstack": ["nova", "neutron"],
...
class NorthSouth(create_resources.CreateResources, osutils.OSScenario, gbputils.GBPScenario, neutron_utils.NeutronScenario,
    ...
    def run(self, controller_ip, image, flavor, L3OUT1, L3OUT1_NET, L3OUT1_VRF, L3OUT2,
            L3OUT2_NET, L3OUT2_VRF, ext_rtr, extrtr_ip1, extrtr_ip2, gwip1_extrtr, gwip2_extrtr, nova_az, plugin_type):
        ...
        print "Execution of Testcase TEST_NAT_FUNC_6 starts\n"
        ...
        command1 = self.command_for_icmp_tcp_traffic_from_ext_rtr(fip1["ip"])
        command2 = self.command_for_icmp_tcp_traffic_from_ext_rtr(fip2["ip"])
        ...
        self.admin_delete_floating_ip(vm1, fip1)
        self.admin_delete_floating_ip(vm2, fip2)
        self.sleep_between(10, 15)

        print "Delete existing nat pool\n"
        self.delete_gbp_nat_pool(gbp, nat_pool)

        print "Create a new NAT pool and associate with External Segment\n"
        nat_pool2 = self.create_gbp_nat_pool(gbp, "GbpNatPoolTest2",
                                            **{"ip_pool": "60.60.60.0/24", "external_segment_id": ext_seg})
        self.sleep_between(50, 60)

        print "Associate FIPs to VMs\n"
        fip3 = self.admin_attach_floating_ip(vm1, ext_net_list[0].get("network"))
        fip4 = self.admin_attach_floating_ip(vm2, ext_net_list[0].get("network"))
        self.sleep_between(10, 15)

        self.add_route_in_extrtr(ext_rtr, "60.60.60.0/24", gwip1_extrtr, "update")

        print "DNATed Traffic from ExtRTR to VMs with FIPs from New NAT-Pool\nICMP and TCP traffic from external router\n"
        command3 = self.command_for_icmp_tcp_traffic_from_ext_rtr(fip3["ip"])
        command4 = self.command_for_icmp_tcp_traffic_from_ext_rtr(fip4["ip"])
        ...
        self.add_route_in_extrtr(ext_rtr, "60.60.60.0/24", gwip2_extrtr, "update")
        self.cleanup_ns(gbp, [vm1, vm2], [fip3, fip4], ext_net_list)
