...
@scenario.configure(name="ScenarioPlugin.north_south_test12", context={"cleanup@openstack": ["nova", "neutron"],
...
class NorthSouth(create_resources.CreateResources, osutils.OSScenario, gbputils.GBPScenario, neutron_utils.NeutronScenario,
    ...
    def run(self, controller_ip, image, flavor, L3OUT1, L3OUT1_NET, L3OUT1_VRF, L3OUT2,
        ...
        try:
            ...
            print "Execution of Testcase TEST_NAT_FUNC_12 starts\n"
            ...
            print "Disassociate FIPs from TestVM2\n"
            self.admin_delete_floating_ip(vm2, fip2)

            print "Associate explicitly created l3p with new ExtSeg\n"
            self.update_gbp_l3policy(gbp, l3p, "uuid", **{"external_segments": ext_seg2})

            print " DNATed Traffic from ExtRTR to the ONLY VM with FIP\n"
            self._remote_command_wo_server("noiro", "noir0123", ext_rtr, command1)

            print "SNATed Traffic from the ONLY VM without FIP to ExtRtr\n"
            self.update_gbp_nat_pool(gbp, nat_pool, **{"external_segment_id": ext_seg2})

            fip2 = self.admin_attach_floating_ip(vm2, ext_net_list[1].get("network"))
            ...
            print "Sending Traffic from TestVM2 to external router"
