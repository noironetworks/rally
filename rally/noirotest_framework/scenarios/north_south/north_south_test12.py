...
class NorthSouth(neutron_utils.NeutronScenario, gbputils.GBPScenario, osutils.OSScenario, nova_utils.NovaScenario,
    ...
    def run(self, controller_ip, image, flavor):
        ...
        ext_net_list, ext_sub_list, vm_list, l3p, defaultl3p, nat_pool, prs_icmp_tcp_id = self.create_resources_for_north_south_tests(
            gbp_ad, gbp, key_name, image, flavor, external_seg2=True)
        ...
        ext_seg2 = self.create_gbp_external_segment(gbp_ad, "L3OUT2", **{"subnet_id": ext_sub_list[2].id, "external_routes": [
        ...
        ##################################################
        # traffic verification from vms to ext-router
        ####################################
