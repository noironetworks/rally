...
class NorthSouth(neutron_utils.NeutronScenario, gbputils.GBPScenario, osutils.OSScenario, nova_utils.NovaScenario,
    ...
    def run(self, controller_ip, image, flavor):
        ...
        ext_net_list, ext_sub_list, vm_list, ext_pol1, ext_seg, icmp_tcp_id = self.create_resources_for_north_south_tests(
                                                                                      gbp_ad, gbp, key_name, image, flavor, ext_pol2=True)
        ...
        self.delete_gbp_external_policy(gbp, ext_pol1)
        ext_pol2 = self.create_gbp_external_policy(gbp, "L3OUT1_NET", **{"external_segments": [ext_seg]})
        self.update_gbp_external_policy(gbp, ext_pol2, "uuid", [icmp_tcp_id])
