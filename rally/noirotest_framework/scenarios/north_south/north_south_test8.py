...
@scenario.configure(name="ScenarioPlugin.north_south_test8", context={"cleanup@openstack": ["nova", "neutron"],
...
class NorthSouth(create_resources.CreateResources, osutils.OSScenario, gbputils.GBPScenario, neutron_utils.NeutronScenario,
    ...
    def run(self, controller_ip, image, flavor, L3OUT1, L3OUT1_NET, L3OUT1_VRF, L3OUT2,
        ...
        print "Execution of Testcase TEST_NAT_FUNC_8 starts\n"
        ...
        print "Delete existing ExtPolicy and create ExtPolicy with ExtSegment and Apply PolicyRuleSets\n"
        self.delete_gbp_external_policy(gbp, ext_pol)

        ext_pol2 = self.create_gbp_external_policy(gbp, L3OUT1_NET, **{"external_segments": [ext_seg]})
        self.update_gbp_external_policy(gbp, ext_pol2, "uuid", [policy_rule_set["icmp_tcp_id"]])
        self.sleep_between(20, 25)

        print "DNATed Traffic from ExtRTR to VMs after ExtPol is re-created\nICMP and TCP traffic from external router\n"
        self._remote_command_wo_server("noiro", "noir0123", ext_rtr, command1)
        self._remote_command_wo_server("noiro", "noir0123", ext_rtr, command2)
