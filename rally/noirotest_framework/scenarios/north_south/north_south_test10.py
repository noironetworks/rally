...
@scenario.configure(name="ScenarioPlugin.north_south_test10", context={"cleanup@openstack": ["nova", "neutron"],
...
class NorthSouth(create_resources.CreateResources, osutils.OSScenario, gbputils.GBPScenario, neutron_utils.NeutronScenario,
    ...
    def run(self, controller_ip, image, flavor, L3OUT1, L3OUT1_NET, L3OUT1_VRF, L3OUT2,
        ...
        try:
            ...
            print "Execution of Testcase north_south_test10 starts\n"
            ...
            self.verify_ns_gbp_object(gbp, defaultl3p, l3p, l2p, ext_seg, "20.20.20.0/24", ext_pol, nat_pool, ptg2, "snat")
            ...
            command5 = self.command_for_icmp_tcp_traffic_from_ext_rtr(fip1["ip"])
            command6 = self.command_for_icmp_tcp_traffic_from_ext_rtr(fip2["ip"])
            self._remote_command_wo_server("noiro", "noir0123", ext_rtr, command5)
            self._remote_command_wo_server("noiro", "noir0123", ext_rtr, command6)
